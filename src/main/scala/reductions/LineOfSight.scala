...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    for {x <- input.indices} {
      if (x == 0) {
        ...
        output(x) = max(input(x) / x, output(x - 1))
      }
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    val tangenses = for {i <- from until `until`} yield {
      if (i == 0) 0
      else {
        input(i) / i
      }
    }
    if (tangenses.isEmpty) 0 else tangenses.max
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val mid = (end - from) / 2
      val (a, b) = parallel(upsweep(input, from, from + mid, threshold), upsweep(input, from + mid, end, threshold))
      Node(a, b)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from < until) {
      val maxAngel = max(input(from) / from, startingAngle)
      output(from) = maxAngel
      downsweepSequential(input, output, maxAngel, from + 1, until)
